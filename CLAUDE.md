...
- **Conducta suicida (ASQ)**: el resultado son **solo dos valores** —`Negativo` / `Positivo`— y lo deciden los 4 ítems 0/1 (`ResponderTamizaje::NIVELES_SUICIDIO`). Es literal el recuadro "PARA LA REVISIÓN" de Angélica; el texto largo que lo acompaña es la *acción* (`ACCIONES_SUICIDIO`), no parte del nombre. **No volver a agregar un tercer valor a esta columna:** hubo `Evaluación Adicional` y después `Riesgo Agudo`, y ella reportó ambos. Ojo: desde el 27/08/2026 la UI **muestra** "Positivo: Riesgo Agudo" y la acción correspondiente junto al resultado — eso es texto de pantalla que sale de [ResultadoAsq](app/Support/ResultadoAsq.php) (ella lo pidió así, con la acción "en letras más chiquitas"); la columna sigue guardando solo dos valores y eso no es una regresión. La **pregunta 5** ("¿Estás pensando en quitarte la vida en este momento?") solo se despliega si hubo algún "Sí" y **no cambia este resultado**: su efecto es subir la prioridad de atención a `Urgente`. El instrumento se queda en **cinco preguntas y ninguna más**: la derivada de la 4 ("¿Cuándo fue el último intento?", menos/más de 12 meses) existió del 21 al 25/08/2026 y **Angélica la retiró** porque, a esas alturas de la aplicación, la habrían contestado muy pocos y no serviría para reportar. **No reponerla sin que ella lo pida.** Los tamizajes que la alcanzaron a contestar conservan el dato en `respuestas['conducta_suicida']['ultimo_intento']` y `TamizajeResource` lo sigue mostrando cuando viene lleno; nunca alteró la clasificación.
- `nivel_riesgo_general` ya **no** es "riesgo general" sino la **prioridad de atención**, y su escala vive completa en [PrioridadAtencion](app/Support/PrioridadAtencion.php) — `calcular()` es la única implementación y la usan tanto el cuestionario como el comando de reclasificación. **No duplicar la tabla:** `Urgente` (ASQ positivo con la 5 en "Sí"), `Alta` (ASQ positivo no agudo, o ansiedad Grave / depresión Mod-grave o Grave), `Moderada` (ansiedad o depresión Moderada), `Leve` (todo mínimo o Leve — Angélica corrigió el 26/08/2026 su tabla original, que ponía los leves en Moderada y le inflaba ese nivel), más `Agudeza pendiente de confirmar` para los tamizajes aplicados antes de que existiera la pregunta 5 — ASQ positivo con agudeza desconocida, que no se puede afirmar ni descartar por recálculo.

La etiqueta visible, los colores y la nota legal ("clasificación operativa del Distintivo, no clínica") salen también de esa clase; los colores de nivel de los instrumentos están en [ColorNivel](app/Support/ColorNivel.php). Cualquier listado, filtro o gráfica nueva debe leer de ahí en vez de escribir los niveles a mano.

`php artisan tamizajes:reclasificar` recalcula los históricos con la escala vigente. Va por pasadas, cada una con su marca en el JSON `respuestas` (`reclasificacion_18_08_2026`, `prioridad_atencion_21_08_2026`) que la vuelve idempotente y deja el nivel anterior para auditoría. **Ninguna pasada eleva un histórico a riesgo agudo**: eso lo determina la valoración clínica.

Los **renombres de valores ya guardados** van como migración, no como pasada del comando (`renombra_riesgo_general_a_prioridad_atencion`, `renombra_resultado_asq_a_positivo_negativo`): el despliegue en cPanel ya corre `migrate --force`, así que es un paso manual menos que se puede olvidar en el servidor.
...
- **La suite de tests está desfasada:** al 21/08/2026 fallan **10 de 126** en `qa-server` —los cinco de `AutoevaluacionStatsWidgetTest`, `DownloadAcusePdfTest`, `EmpresaDistintivoExpositionTest` y los tres de `RegistroEmpresaTest`— por cambios de dominio que nunca se reflejaron en los tests (etiquetas `Rechazado` → `no_cumple`, umbrales de nivel de madurez, `canAccessPanel` que ahora exige `role` + `estatus`, registro público cerrado). Al tocar código, compara contra ese baseline en vez de asumir que la suite estaba verde.
- **`vendor/bin/pint` sin ruta reformatea todo el repo:** el código no está limpio de Pint (sobre todo el orden de los `use`), así que correrlo pelado ensucia ~80 archivos ajenos al cambio. Pásale siempre los archivos tocados: `vendor/bin/pint app/Support/PrioridadAtencion.php …`.
- **`php artisan test` se queda sin memoria** al llegar a `AutoevaluacionStatsWidgetTest` (el formulario de la autoevaluación son ~800 líneas de esquema). Correr la suite con `php -d memory_limit=1G vendor/bin/phpunit`.
