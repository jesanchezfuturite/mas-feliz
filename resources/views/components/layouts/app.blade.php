    ...
    <header class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800/80 sticky top-0 z-40">
        <div class="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
            <div class="flex items-center space-x-4">
                ...
                @endphp
                @if($partnerLogo)
                    <img src="{{ Storage::disk('public')->url($partnerLogo) }}" alt="Partner" class="h-24 w-auto border-r border-slate-200 dark:border-slate-700 pr-4" />
                @endif
                <img src="{{ asset('images/+FELIZ-LOGO.png') }}" alt="+Feliz" class="h-28 w-auto" />
                <span class="h-16 w-px bg-slate-200 dark:bg-slate-700"></span>
                <span class="text-[12px] font-bold text-slate-400 uppercase tracking-widest mt-1">Diagnóstico</span>
            </div>
